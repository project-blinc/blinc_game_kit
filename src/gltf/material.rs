...
fn compress_textures(
    ...
    let new_base = base_color.as_ref().and_then(|td| {
        let kind = match resolved_alpha_mode {
            AlphaMode::Opaque => super::bc_encode::encode_bc1,
            AlphaMode::Mask | AlphaMode::Blend => super::bc_encode::encode_bc3,
        };
        encode_one(td, kind)
    });
    let new_normal = normal
        .as_ref()
        .and_then(|td| encode_one(td, super::bc_encode::encode_bc5_rg));
    let new_occl = occlusion
        .as_ref()
        .and_then(|td| encode_one(td, super::bc_encode::encode_bc4_red));
    (
